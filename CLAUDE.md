...
- `skills/` — SKILL.md slash commands. Invokable standalone or referenced by agents.
  - `skills/shared/` — Domain-agnostic skills available to any agent.
  - `skills/professional/` — Skills scoped to professional work.
  - `skills/personal/` — Skills scoped to personal use.
- `memory/` — Persistent learnings written by skills like `retrospective` and `memory-update`.

## Conventions

### Agents
- Agent files live at `agents/{personal|professional}/{name}.md`
...
### Skills
- Each skill lives in its own directory: `skills/{scope}/{skill-name}/SKILL.md`
